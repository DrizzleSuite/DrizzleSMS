...
import org.grovecity.drizzlesms.mms.LegacyMmsConnection;
...
public class ApnDatabase {
  ...
  private LegacyMmsConnection.Apn getCustomApnParameters() {
      String mmsc = DrizzleSmsPreferences.getMmscUrl(context).trim();

      if (!TextUtils.isEmpty(mmsc) && !mmsc.startsWith("http"))
        mmsc = "http://" + mmsc;

      String proxy = DrizzleSmsPreferences.getMmscProxy(context);
      String port  = DrizzleSmsPreferences.getMmscProxyPort(context);
      String user  = DrizzleSmsPreferences.getMmscUsername(context);
      String pass  = DrizzleSmsPreferences.getMmscPassword(context);
  ...
  public Optional<LegacyMmsConnection.Apn> getMmsConnectionParameters(String mccmnc, String apn) {
    LegacyMmsConnection.Apn customApn  = getCustomApnParameters();
    LegacyMmsConnection.Apn defaultApn = getDefaultApnParameters(mccmnc, apn);
    LegacyMmsConnection.Apn result     = new LegacyMmsConnection.Apn(customApn, defaultApn,
                             DrizzleSmsPreferences.getUseCustomMmsc(context),
                             DrizzleSmsPreferences.getUseCustomMmscProxy(context),
                             DrizzleSmsPreferences.getUseCustomMmscProxyPort(context),
                             DrizzleSmsPreferences.getUseCustomMmscUsername(context),
                             DrizzleSmsPreferences.getUseCustomMmscPassword(context));
