...
import org.grovecity.drizzlesms.database.documents.IdentityKeyMismatchList;
import org.grovecity.drizzlesms.database.documents.NetworkFailure;
import org.grovecity.drizzlesms.database.documents.NetworkFailureList;
...
import org.grovecity.drizzlesms.database.model.NotificationMmsMessageRecord;
import org.grovecity.drizzlesms.jobs.TrimThreadJob;
...
import org.grovecity.drizzlesms.mms.TextSlide;
...
public class MmsDatabase extends MessagingDatabase {
      ...
      while (cursor.moveToNext()) {
        if (Types.isOutgoingMessageType(cursor.getLong(cursor.getColumnIndexOrThrow(MESSAGE_BOX)))) {
          ...
          for (String storedAddress : addresses) {
            try {
              String ourAddress   = Util.canonicalizeNumber(context, address);
  ...
  private long getThreadIdFor(IncomingMediaMessage retrieved) throws RecipientFormattingException, MmsException {
    ...
    try {
      ...
      if (localNumber == null) {
          localNumber = DrizzleSmsPreferences.getLocalNumber(context);
      }
  ...
  public void markAsPendingIndrizzlesmsFallback(long messageId) {
