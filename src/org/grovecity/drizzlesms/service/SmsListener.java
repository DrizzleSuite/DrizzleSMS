...
import org.grovecity.drizzlesms.jobs.SmsReceiveJob;
...
public class SmsListener extends BroadcastReceiver {
  ...
  private static final Pattern CHALLENGE_PATTERN = Pattern.compile(".*Your DrizzleSMS verification code: ([0-9]{3,4})-([0-9]{3,4}).*");
...
//  }
  ...
  private boolean isRelevant(Context context, Intent intent) {
    ...
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT &&
        DrizzleSmsPreferences.isInterceptAllSmsEnabled(context))
    {
      return true;
    }
  ...
  private boolean isChallenge(Context context, Intent intent) {
    ...
    if (CHALLENGE_PATTERN.matcher(messageBody).matches() &&
        DrizzleSmsPreferences.isVerifying(context))
    {
      return true;
    }
